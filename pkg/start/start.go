...
func (b *startCommand) Run() error {
	...
	if _, err := client.CoreV1().Events("kube-system").Create(ctx, makeBootstrapSuccessEvent("kube-system", "bootstrap-success"), metav1.CreateOptions{}); err != nil && !apierrors.IsAlreadyExists(err) {
		return err
	}
	...
	if _, err := client.CoreV1().Events("kube-system").Create(ctx, makeBootstrapSuccessEvent("kube-system", "bootstrap-finished"), metav1.CreateOptions{}); err != nil && !apierrors.IsAlreadyExists(err) {
		return err
	}
